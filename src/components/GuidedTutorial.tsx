...
const STEPS: TutorialStep[] = [
  { title: 'AtoZ 따라오세요 👋', text: '아무것도 몰라도 됩니다. 화면의 손가락과 안내문만 따라오세요. 실제 버튼을 누르면 다음 단계로 자동 진행됩니다.' },
  { title: '1. 새 프로젝트 만들기', text: '오른쪽 위의 “새 프로젝트” 버튼을 클릭하세요. 사업 아이디어를 입력하는 위저드가 열립니다.', selector: '#new-project-btn' },
  { title: '2. 사업화 위저드 확인', text: '상단의 “사업화 위저드” 메뉴를 클릭하세요. 프로젝트명, 아이디어, 고객, 판매채널 등을 순서대로 입력합니다.', selector: '#nav-wizard' },
  { title: '3. 시장조사로 아이디어 검증', text: '“A to Z 시장조사”를 클릭하세요. 경쟁사, 시장성, 수익모델, 위험을 확인합니다.', selector: '#nav-market' },
  ...
  { title: '완료했습니다 🎉', text: 'AtoZ의 전체 흐름을 한 번 경험했습니다. 다시 막히면 상단의 “사용자 매뉴얼”에서 검색하거나 “따라하기”를 다시 시작하세요.' },
];
  ...
  return (
    <div className="fixed inset-0 z-[100] pointer-events-none">
      ...
      {rect && (
        <>
          <div
            className="fixed rounded-2xl border-4 border-yellow-300 pointer-events-none animate-pulse"
            style={{
              left: rect.left - 8,
              top: rect.top - 8,
              width: rect.width + 16,
              height: rect.height + 16,
              boxShadow: '0 0 0 6px rgba(37,99,235,0.55), 0 0 40px rgba(250,204,21,0.9)',
            }}
          />
          <div
            className="fixed text-5xl z-[102] pointer-events-none drop-shadow-lg animate-bounce"
            style={{ left: Math.max(8, rect.left + rect.width / 2 - 20), top: Math.max(8, rect.top - 58) }}
          >
            👇
          </div>
        </>
      ...
      <section
        className="fixed z-[104] bg-white rounded-3xl border-2 border-slate-900 p-5 pointer-events-auto"
        style={{ ...cardStyle, boxShadow: '6px 6px 0 rgba(37,99,235,1)' }}
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="inline-flex items-center gap-1.5 text-[10px] font-black text-blue-700 bg-blue-50 px-2.5 py-1 rounded-full">
              ✨ 초보자 따라하기 {index + 1}/{STEPS.length}
            </div>
            <h3 className="mt-2 text-lg font-black text-slate-900">{step.title}</h3>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center hover:bg-slate-200" aria-label="튜토리얼 닫기">✕</button>
        </div>
        ...
        {step.selector && (
          <div className="mt-4 flex items-center gap-2 text-sm font-black text-amber-700 bg-amber-50 border border-amber-200 rounded-2xl p-3">
            ...
            <span>손가락이 가리키는 버튼을 직접 클릭하세요.</span>
          </div>
        )}

        <div className="mt-5 flex items-center justify-between gap-2">
          <button disabled={index === 0} onClick={() => setIndex((i) => Math.max(0, i - 1))} className="px-3 py-2 rounded-full text-xs font-black text-slate-600 bg-slate-100 disabled:opacity-30">← 이전</button>
          <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-blue-600" style={{ width: `${((index + 1) / STEPS.length) * 100}%` }} />
          </div>
          {index < STEPS.length - 1 ? (
            <button onClick={() => setIndex((i) => Math.min(STEPS.length - 1, i + 1))} className="px-3 py-2 rounded-full text-xs font-black text-white bg-slate-900">건너뛰기 →</button>
          ) : (
