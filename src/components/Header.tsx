...
export const Header: React.FC<HeaderProps> = ({
  ...
  const tabs = [
    ...
    ...(user?.role === 'admin' ? [{ id: 'admin', label: '관리자' }] : []),
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between min-h-18 py-2 gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            {/* 사용자 매뉴얼 / 따라하기 버튼 (from main) */}
            <button
              id="manual-btn"
              onClick={() => onSelectTab('guide')}
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-full border border-slate-200 bg-white text-xs font-black text-slate-700 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-blue-500 focus-visible:outline-offset-2"
              title="검색 가능한 사용자 매뉴얼"
              aria-label="사용자 매뉴얼"
            >
              <span aria-hidden="true">📘</span>
              <span className="hidden xl:inline">사용자 매뉴얼</span>
            </button>
            <button
              id="tutorial-btn"
              onClick={onStartTutorial}
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-full border border-amber-300 bg-amber-50 text-xs font-black text-amber-800 hover:bg-amber-100 focus-visible:outline-2 focus-visible:outline-amber-500 focus-visible:outline-offset-2"
              title="처음부터 따라하는 초보자 튜토리얼"
              aria-label="따라하기 튜토리얼 시작"
            >
              <span aria-hidden="true">👉</span>
              <span className="hidden xl:inline">따라하기</span>
            </button>
            ...
            {/* Auth buttons (from feature branch) */}
            {status !== 'loading' && !user && (
              <div className="flex items-center gap-2">
                <button
                  onClick={onOpenLogin}
                  aria-label="로그인"
                  className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-slate-600 hover:text-slate-900 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors focus-visible:outline-2 focus-visible:outline-blue-500"
                >
                ...
                <button
                  ...
                  aria-label="회원가입"
                  className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-colors focus-visible:outline-2 focus-visible:outline-blue-500"
                >
            ...
            {user && (
              <button
                ...
                aria-label="내 계정"
                className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-50 transition-colors border border-slate-200 focus-visible:outline-2 focus-visible:outline-blue-500"
              >
                {user.plan === 'pro' ? (
          ...
          <nav aria-label="주요 메뉴" className="flex space-x-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <span className={`w-2 h-2 rounded-full ${isActive ? 'bg-blue-600' : 'bg-slate-200'}`} aria-hidden="true" />
                  <span>{tab.label}</span>
                </button>
              );
            })}
